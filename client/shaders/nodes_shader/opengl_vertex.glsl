...
void main(void)
{
    ...
    //	vec4 color = inVertexAmbientColor;
    //    color.rgb = color.rgb - abs(1.0 - inVertexAmbientColor.rgb) / 4.0;
    ...
    //    nightRatio * artificialLight.rgb) * 1.0;
    //    color.a = 1.0;
...
/* normalOffsetScale is in world coordinates (1/10th of a meter)
    ...
    if (f_timeofday < 0.2) {
        ...
        (1.0 - smoothstep(0.18, 0.2, f_timeofday));
    } else if (f_timeofday >= 0.8) {
        adj_shadow_strength = f_shadow_strength * 0.5 *
        smoothstep(0.8, 0.83, f_timeofday);
    } else {
        adj_shadow_strength = f_shadow_strength *
        smoothstep(0.20, 0.25, f_timeofday) *
        (1.0 - smoothstep(0.7, 0.8, f_timeofday));
    }
}
    #endif
}
